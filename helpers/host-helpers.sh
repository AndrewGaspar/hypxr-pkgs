# Resolving the HypXR repository host.
#
# One machine holds the complete HypXR repository and runs scheduled builds. The
...
resolve_repo_host() {
  ...
  if [[ -n "${HYPXR_REPO_HOST:-}" ]]; then
    echo "$HYPXR_REPO_HOST"
    return 0
  fi
...
on_repo_host() {
  [[ -f "$REPO_DIR/hypxr.db" || -f "$REPO_DIR/hypxr.db.tar.zst" ]]
}
...
print_no_repo_host() {
  ...
  echo ""
  echo "Pass --host, set HYPXR_REPO_HOST, or write the destination to:"
